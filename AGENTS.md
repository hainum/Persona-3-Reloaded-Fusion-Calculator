...
- **Search is depth-unlimited**: Worker iterates depths 1..20 (`MAX_DEPTH`), stopping when 5 consecutive depths yield zero paths. No user-facing depth control — all unique paths found are returned progressively.
...
- **No "See Deeper Paths" button**: Replaced by exhaustive worker search with progressive result delivery at each depth boundary.
...
- **No sub-problem caps** — `searchTree` and `generateFusionTrees` return all valid paths without limit. **Worker caps at `MAX_UNIQUE_PATHS = 200`** total unique paths to prevent UI overload. Benchmarked as Strategy D: best coverage without rendering thousands of paths.

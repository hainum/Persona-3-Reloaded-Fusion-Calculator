...
| **Required Personas Filter** | Optionally constrain results to paths that contain specific Personas. |
...
| **Progressive Depth** | Start with a shallow search and click *See Deeper Paths* to incrementally widen the search space. Floating sticky button when scrolled. |
| **Persona Database** | Browse all Personas with detail views showing resistances, innate/learned skills, and fusion recipes. |
| **Skill Database** | Browse all persona-learnable skills with effect descriptions, SP cost, minimum level, and learned-by list. |
| **Bookmarks** | Save and load fusion configurations via localStorage-persisted bookmarks. |
| **Persistent Settings** | Current level is persisted to `localStorage` across sessions. |
| **Glassmorphism UI** | Dark-mode, game-themed interface with searchable dropdowns (keyboard-navigable), animated tree visualisation, and responsive layout. |
...
5. **Go deeper** — If no paths are found (or you want more options), click **See Deeper Paths** to increase the search depth by 1. A floating `+` button appears when the main button is scrolled out of view.
6. **Browse Database** — Switch to the Database tab to explore Personas and skills. Click any Persona for a detail view with resistances, skills, and fusion recipes.
7. **Bookmark** — Save any configuration from the Calculator or Database for quick recall later.
...
│   ├── components/
│   │   ├── SearchableSelect.jsx  # Searchable dropdown with keyboard navigation
│   │   ├── FusionPathViewer.jsx  # Recursive tree renderer for fusion paths
│   │   ├── PersonaDatabase.jsx   # Persona + skill list tables, detail views, bookmark CTAs
│   │   ├── BookmarkDrawer.jsx    # Slide-in drawer listing bookmarks with load/delete
│   │   └── BookmarkModal.jsx     # SaveBookmarkModal + AddSkillToBookmarkModal
│   ├── data/
...
│   │   ├── fusion-chart.json     # Lower-triangular arcana x arcana fusion table
...
│       └── BookmarkManager.js    # Bookmark CRUD, localStorage persistence, matching
├── tests/
│   ├── run.js                    # Test runner (forks all test files)
│   ├── algorithm.test.js         # Algorithm + data parsing + effect description tests
│   └── bookmark.test.js          # BookmarkManager unit tests
└── docs/
...
- **Lucide React** — Icon set (Zap, Search, ChevronDown, Check, Calculator, Database, Bookmark, Lock, Star, Plus, etc.)
